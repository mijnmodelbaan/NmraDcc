...
#define ACTIVE_OUTPUT_STATE HIGH    // Set the ACTIVE State to Drive the Turnout motor HIGH or LOW 
...
CVPair FactoryDefaultCVs [] =
{
  ...
  {CV_ACCESSORY_DECODER_OUTPUT_PULSE_TIME, 25},   // x 10mS for the output pulse duration
  {CV_ACCESSORY_DECODER_CDU_RECHARGE_TIME, 25},   // x 10mS for the CDU recharge delay time
...
byte outputs[] = { 2, 3, 4, 5, 7, 8, 9, 10, 11, 12, 14, 15, 16, 17, 18, 19};
//   pins         D2 D3 D4 D5 D7 D8 D9 D10 D11 D12  A0  A1  A2  A3  A4  A5  
...
void setup()
{
  ...
  pinMode(13, OUTPUT);    // sets the digital pin B5 as output

  // Setup which External Interrupt, the Pin it's associated with that we're using 
  Dcc.pin(digitalPinToInterrupt(DCC_PIN), DCC_PIN, 0);
  
  // Call the main DCC Init function to enable the DCC Receiver
  Dcc.init( MAN_ID_DIY, DCC_DECODER_VERSION_NUM, FLAGS_AUTO_FACTORY_DEFAULT | FLAGS_CV29_BITS, 0);
...
void notifyCVChange(uint16_t CV, uint8_t Value)
{
  ...
  if ((CV == CV_ACCESSORY_DECODER_ADDRESS_MSB)       ||
      (CV == CV_ACCESSORY_DECODER_ADDRESS_LSB)       ||
